      ...
      axios.get('/api/ratings')
        .then(function (response) {
          if (response.data.errno === ERR_OK) {
            var ratings = response.data.data
...
<style>
  ...
  .ratings .overview .overview-right .deliveryTime .title {
    font-size: 12px;
    color: rgb(7, 17, 27);
    line-height: 18px;
  }

  .ratings .overview .overview-right .deliveryTime .delivery {
    font-size: 12px;
    color: rgb(147, 153, 159);
    margin-left: 12px;
  }
  ...
  .ratings .rating-wrapper .rating-item .recommend .icon-thumb_up,.ratings .rating-wrapper .rating-item .recommend .item{
    display: inline-block;
    margin:0 8px 4px 0 ;
    font-size: 10px;
  }
